...
AffixMgr::AffixMgr(const char * affpath, HashMgr** ptr, int * md, const char * key) 
{
  ...
  if (parse_file(affpath, key)) {
     HUNSPELL_WARNING(hunspell_stderr, "Failure loading aff file %s\n",affpath);
  }
...
AffixMgr::~AffixMgr() 
{
  ...
  if (keystring) op_free(keystring);
  keystring=NULL;
  if (trystring) op_free(trystring);
  trystring=NULL;
  ...
  if (maptable) {  
     for (int j=0; j < nummap; j++) {
        for (int k=0; k < maptable[j].len; k++) {
           if (maptable[j].set[k]) op_free(maptable[j].set[k]);
        }
        op_free(maptable[j].set);
        maptable[j].set = NULL;
        maptable[j].len = 0;
     }
     op_free(maptable);  
  ...
  if (breaktable) {
     for (int j=0; j < numbreak; j++) {
        if (breaktable[j]) op_free(breaktable[j]);
        breaktable[j] = NULL;
     }
     op_free(breaktable);  
     breaktable = NULL;
  ...
  if (reptable) {
     for (int j=0; j < numrep; j++) {
        op_free(reptable[j].pattern);
        op_free(reptable[j].pattern2);
     }
     op_free(reptable);  
     reptable = NULL;
  }
  if (iconvtable) OP_DELETE( iconvtable);
  if (oconvtable) OP_DELETE( oconvtable);
  if (phone && phone->rules) {
     for (int j=0; j < phone->num + 1; j++) {
        op_free(phone->rules[j * 2]);
        op_free(phone->rules[j * 2 + 1]);
     }
     op_free(phone->rules);
     op_free(phone);  
     phone = NULL;
  ...
  if (checkcpdtable) {  
     for (int j=0; j < numcheckcpd; j++) {
        op_free(checkcpdtable[j].pattern);
        op_free(checkcpdtable[j].pattern2);
        op_free(checkcpdtable[j].pattern3);
        checkcpdtable[j].pattern = NULL;
        checkcpdtable[j].pattern2 = NULL;
        checkcpdtable[j].pattern3 = NULL;
     }
     op_free(checkcpdtable);  
     checkcpdtable = NULL;
  }
  numcheckcpd = 0;
  ...
  cpdmin = 0;
  cpdmaxsyllable = 0;
  if (cpdvowels) op_free(cpdvowels);
  if (cpdvowels_utf16) op_free(cpdvowels_utf16);
  if (cpdsyllablenum) op_free(cpdsyllablenum);
#ifdef HUNSPELL_USE_UTF_TBL
  free_utf_tbl();
#endif
  if (lang) op_free(lang);
  if (wordchars) op_free(wordchars);
  if (wordchars_utf16) op_free(wordchars_utf16);
  if (ignorechars) op_free(ignorechars);
  if (ignorechars_utf16) op_free(ignorechars_utf16);
  if (version) op_free(version);
  checknum=0;
...
int  AffixMgr::parse_file(const char * affpath, const char * key)
{
  ...
  char dupflags[CONTSIZE]; // ARRAY OK 2009-02-09 jb
  char dupflags_ini = 1;
  ...
  // affix data structures
    ...
    while ((line = afflst->getline())) {
       ...
       if (firstline) {
         ...
         if (op_strncmp(line,"\xEF\xBB\xBF",3) == 0) {
            op_memmove(line, line+3, op_strlen(line+3)+1);
         }
       }

       /* parse in the keyboard string */
       if (op_strncmp(line,"KEY",3) == 0) {
       ...
       if (op_strncmp(line,"COMPLEXPREFIXES",15) == 0)
                   complexprefixes = 1;

       /* parse in the flag used by the controlled compound words */
       if (op_strncmp(line,"COMPOUNDFLAG",12) == 0) {
       ...
       if (op_strncmp(line,"COMPOUNDBEGIN",13) == 0) {
          if (complexprefixes) {
       ...
       /* parse in the flag used by compound words */
       if (op_strncmp(line,"COMPOUNDMIDDLE",14) == 0) {
       ...
       /* parse in the flag used by compound words */
       if (op_strncmp(line,"COMPOUNDEND",11) == 0) {
          if (complexprefixes) {
            if (parse_flag(line, &compoundbegin, afflst)) {
              OP_DELETE( afflst);
              return 1;
       ...
       if (op_strncmp(line,"COMPOUNDWORDMAX",15) == 0) {
       ...
       if (op_strncmp(line,"COMPOUNDROOT",12) == 0) {
       ...
       if (op_strncmp(line,"COMPOUNDPERMITFLAG",18) == 0) {
       ...
       /* parse in the flag used by compound_check() method */
       if (op_strncmp(line,"COMPOUNDFORBIDFLAG",18) == 0) {
       ...
       if (op_strncmp(line,"CHECKCOMPOUNDDUP",16) == 0) {
                   checkcompounddup = 1;
       }

       if (op_strncmp(line,"CHECKCOMPOUNDREP",16) == 0) {
                   checkcompoundrep = 1;
       }

       if (op_strncmp(line,"CHECKCOMPOUNDTRIPLE",19) == 0) {
                   checkcompoundtriple = 1;
       }

       if (op_strncmp(line,"SIMPLIFIEDTRIPLE",16) == 0) {
                   simplifiedtriple = 1;
       }

       if (op_strncmp(line,"CHECKCOMPOUNDCASE",17) == 0) {
                   checkcompoundcase = 1;
       }

       if (op_strncmp(line,"NOSUGGEST",9) == 0) {
       ...
       if (op_strncmp(line,"NONGRAMSUGGEST",14) == 0) {
       ...
       /* parse in the flag used by forbidden words */
       if (op_strncmp(line,"LEMMA_PRESENT",13) == 0) {
       ...
       if (op_strncmp(line,"CIRCUMFIX",9) == 0) {
       ...
       if (op_strncmp(line,"ONLYINCOMPOUND",14) == 0) {
       ...
       if (op_strncmp(line,"PSEUDOROOT",10) == 0) {
       ...
       /* parse in the flag used by `needaffixs' */
       if (op_strncmp(line,"NEEDAFFIX",9) == 0) {
          if (parse_flag(line, &needaffix, afflst)) {
             OP_DELETE( afflst);
             return 1;
          }
       ...
       if (op_strncmp(line,"COMPOUNDMIN",11) == 0) {
       ...
       if (op_strncmp(line,"COMPOUNDSYLLABLE",16) == 0) {
       ...
       /* parse in the flag used by compound_check() method */
       if (op_strncmp(line,"SYLLABLENUM",11) == 0) {
       ...
       /* parse in the flag used by the controlled compound words */
       if (op_strncmp(line,"CHECKNUM",8) == 0) {
           checknum=1;
       }

       /* parse in the extra word characters */
       if (op_strncmp(line,"WORDCHARS",9) == 0) {
       ...
       if (op_strncmp(line,"CHECKCOMPOUNDPATTERN",20) == 0) {
       ...
       if (op_strncmp(line,"COMPOUNDRULE",12) == 0) {
       ...
       if (op_strncmp(line,"VERSION",7) == 0) {
          for(line = line + 7; *line == ' ' || *line == '\t'; line++);
          version = mystrdup(line);
       }

       if (op_strncmp(line,"MAXNGRAMSUGS",12) == 0) {
       ...
       if (op_strncmp(line,"ONLYMAXDIFF", 11) == 0)
                   onlymaxdiff = 1;

       if (op_strncmp(line,"MAXDIFF",7) == 0) {
       ...
       if (op_strncmp(line,"MAXCPDSUGS",10) == 0) {
       ...
       if (op_strncmp(line,"NOSPLITSUGS",11) == 0) {
                   nosplitsugs=1;
       }

       if (op_strncmp(line,"FULLSTRIP",9) == 0) {
                   fullstrip=1;
       }

       if (op_strncmp(line,"SUGSWITHDOTS",12) == 0) {
                   sugswithdots=1;
       }

       /* parse in the flag used by forbidden words */
       if (op_strncmp(line,"KEEPCASE",8) == 0) {
       ...
       if (op_strncmp(line,"FORCEUCASE",10) == 0) {
       ...
       if (op_strncmp(line,"FORBIDWARN",10) == 0) {
                   forbidwarn=1;
       }

       /* parse in the flag used by the affix generator */
       if (op_strncmp(line,"SUBSTANDARD",11) == 0) {
       ...
       if (op_strncmp(line,"CHECKSHARPS",11) == 0) {
                   checksharps=1;
       }

       /* parse this affix: P - prefix, S - suffix */
       ft = ' ';
       if (op_strncmp(line,"PFX",3) == 0) ft = complexprefixes ? 'S' : 'P';
       if (op_strncmp(line,"SFX",3) == 0) ft = complexprefixes ? 'P' : 'S';
       if (ft != ' ') {
          if (dupflags_ini) {
            op_memset(dupflags, 0, sizeof(dupflags));
            dupflags_ini = 0;
          }
    ...
    char expw[MAXLNLEN];// ARRAY OK 2009-02-09 jb
    if (wordchars) {
        op_strlcpy(expw, wordchars, sizeof(expw));
        op_free(wordchars);
    } else *expw = '\0';

    for (int i = 0; i <= 255; i++) {
        if ( (csconv[i].cupper != csconv[i].clower) &&
            (! op_strchr(expw, (char) i))) {
                *(expw + op_strlen(expw) + 1) = '\0';
                *(expw + op_strlen(expw)) = (char) i;
        }
    }
    ...
    if (numbreak == -1) {
        breaktable = (char **) op_malloc(sizeof(char *) * 3);
...
int AffixMgr::build_pfxtree(PfxEntry* pfxptr)
{
  ...
  for (;;) {
    pptr = ptr;
    if (op_strcmp(ep->getKey(), ptr->getKey() ) <= 0) {
       ptr = ptr->getNextEQ();
       if (!ptr) {
          pptr->setNextEQ(ep);
          break;
...
int AffixMgr::encodeit(affentry &entry, char * cs)
{
  if (op_strcmp(cs,".") != 0) {
    ...
    hunspell_strncpy(entry.c.conds, cs, MAXCONDLEN);
...
{
  char candidate[MAXLNLEN]; // ARRAY OK 2009-02-26 jb
  const char * r;
  int lenr, lenp;
  ...
  for (int i=0; i < numrep; i++ ) {
      ...
      while ((r=op_strstr(r, reptable[i].pattern)) != NULL) {
          if (r-word + lenr + op_strlen(r+lenp) >= MAXLNLEN) break;
          ...
          if (candidate_check(candidate,op_strlen(candidate))) return 1;
  ...
  for (int i = 0; i < numcheckcpd; i++) {
      if (isSubset(checkcpdtable[i].pattern2, word + pos) &&
        ...
        (!*(checkcpdtable[i].pattern) || (
            (*(checkcpdtable[i].pattern)=='0' && r1->blen <= pos && op_strncmp(word + pos - r1->blen, r1->word, r1->blen) == 0) ||
            (*(checkcpdtable[i].pattern)!='0' && (len = op_strlen(checkcpdtable[i].pattern)) &&
                op_strncmp(word + pos - len, checkcpdtable[i].pattern, len) == 0)))) {
            return 1;
        }
  }
  return 0;
...
short AffixMgr::get_syllable(const char * word, int wlen)
{
    ...
    if (!utf8) {
        for (int i=0; i<wlen; i++) {
            if (op_strchr(cpdvowels, word[i])) num++;
        }
    } else if (cpdvowels_utf16) {
        w_char w[MAXWORDUTF8LEN]; // ARRAY OK 2009-02-09 jb
    ...
    for (i = cmin; i < cmax; i++) {
        ...
        if (scpd > 0) {
          for (; scpd <= numcheckcpd && (!checkcpdtable[scpd-1].pattern3 ||
            op_strncmp(word + i, checkcpdtable[scpd-1].pattern3, op_strlen(checkcpdtable[scpd-1].pattern3)) != 0); scpd++);

          if (scpd > numcheckcpd) break; // break simplified checkcompoundpattern loop
          op_strcpy(st + i, checkcpdtable[scpd-1].pattern);
          soldi = i;
          i += op_strlen(checkcpdtable[scpd-1].pattern);
          op_strcpy(st + i, checkcpdtable[scpd-1].pattern2);
          op_strcpy(st + i + op_strlen(checkcpdtable[scpd-1].pattern2), word + soldi + op_strlen(checkcpdtable[scpd-1].pattern3));

          oldlen = len;
          len += op_strlen(checkcpdtable[scpd-1].pattern) + op_strlen(checkcpdtable[scpd-1].pattern2) - op_strlen(checkcpdtable[scpd-1].pattern3);
          oldcmin = cmin;
          oldcmax = cmax;
...
// LANG_hu section: spec. Hungarian rule
            if (langnum == LANG_hu) {
                ...
                if (pfx && (get_syllable(pfx->getKey(),op_strlen(pfx->getKey())) > 1)) wordnum++;
            }
// END of LANG_hu section
...
// END of LANG_hu section
            ...
            sfx = NULL;
            sfxflag = FLAG_NULL;
            rv = (compoundflag && !onlycpdrule) ? affix_check((word+i),op_strlen(word+i), compoundflag, IN_CPD_END) : NULL;
            if (!rv && compoundend && !onlycpdrule) {
                sfx = NULL;
                pfx = NULL;
                rv = affix_check((word+i),op_strlen(word+i), compoundend, IN_CPD_END);
            }

            if (!rv && numdefcpd && words) {
                rv = affix_check((word+i),op_strlen(word+i), 0, IN_CPD_END);
            ...
            if (langnum == LANG_hu) {
                // calculate syllable number of the word
                numsyllable += get_syllable(word + i, op_strlen(word + i));

                // - affix syllable num.
                // XXX only second suffix (inflections, not derivations)
                if (sfxappnd) {
                    char * tmp = myrevstrdup(sfxappnd);
                    numsyllable -= get_syllable(tmp, op_strlen(tmp));
                    op_free(tmp);
                }
            ...
            if (wordnum < maxwordnum) {
                rv = compound_check((st+i),op_strlen(st+i), wordnum+1,
            ...
            if (rv) {
                ...
                if (checkcompoundrep || forbiddenword) {
                    ...
                    // check first part
                    if (op_strncmp(rv->word, word + i, rv->blen) == 0) {
                        char r = *(st + i + rv->blen);
                        *(st + i + rv->blen) = '\0';
                        ...
                        if (forbiddenword) {
                            ...
                            if (rv2 && rv2->astr && TESTAFF(rv2->astr, forbiddenword, rv2->alen) && 
                                (op_strncmp(rv2->word, st, i + rv->blen) == 0)) {
                                    return NULL;
                            }
                        }
                        *(st + i + rv->blen) = r;
        ...
        if (soldi != 0) {
          i = soldi;
          op_strcpy(st, word); // XXX add more optim.
          soldi = 0;
        } else st[i] = ch;
    ...
    char hu_mov_rule = 0, char ** result = NULL, char * partresult = NULL)
{
    ...
    struct hentry * rv = NULL;
    struct hentry * rv_first;
    struct hentry * rwords[MAXWORDLEN]; // ARRAY OK 2009-02-09 jb (NOT CHECKED) // buffer for COMPOUND pattern checking
    char st [MAXWORDUTF8LEN + 4] ;// ARRAY OK 2009-02-09 jb (NOT CHECKED)
    char ch = '\0';

    int checked_prefix;
    char presult[MAXLNLEN]; // ARRAY OK 2009-02-26 jb (NOT CHECKED)
    ...
    for (i = cmin; i < cmax; i++) {
        ...
        *presult = '\0';
        ...
        if (rv)  {
            op_sprintf(presult + op_strlen(presult), "%c%s%s", MSEP_FLD, MORPH_PART, st);
            if (!HENTRY_FIND(rv, MORPH_STEM)) {
                op_sprintf(presult + op_strlen(presult), "%c%s%s", MSEP_FLD, MORPH_STEM, st);
            }
            // store the pointer of the hash entry
//            op_sprintf(presult + op_strlen(presult), "%c%s%p", MSEP_FLD, MORPH_HENTRY, rv);
            if (HENTRY_DATA(rv)) {
                op_sprintf(presult + op_strlen(presult), "%c%s", MSEP_FLD, HENTRY_DATA2(rv));
            }
        }        
        ...
        if (!rv) {
            ...
            if (rv ||
              ...
              ((wordnum > 0) && compoundmiddle &&
                ...
                if (p && (*p != '\0')) {
                    op_sprintf(presult + op_strlen(presult), "%c%s%s%s", MSEP_FLD,
...
// END of LANG_hu section
        ...
        while ((rv) && ((needaffix && TESTAFF(rv->astr, needaffix, rv->alen)) ||
            ...
            if (rv && words && words[wnum + 1]) {
                  ...
                  // store the pointer of the hash entry
//                  op_sprintf(*result + op_strlen(*result), " %s%p", MORPH_HENTRY, rv);
...
//                        op_sprintf(*result + op_strlen(*result), " %s%p", MORPH_HENTRY, rv);
            ...
            if (compoundflag && !onlycpdrule) rv = affix_check((word+i),op_strlen(word+i), compoundflag); else rv = NULL;

            if (!rv && compoundend && !onlycpdrule) {
                sfx = NULL;
                pfx = NULL;
                rv = affix_check((word+i),op_strlen(word+i), compoundend);
            }
            ...
            if (!rv && numdefcpd && words) {
                ...
                if (rv && words && defcpd_check(&words, wnum + 1, rv, NULL, 1)) {
                      char * m = NULL;
                      if (compoundflag) m = affix_check_morph((word+i),op_strlen(word+i), compoundflag);
                      if ((!m || *m == '\0') && compoundend) {
                            if (m) op_free(m);
                            m = affix_check_morph((word+i),op_strlen(word+i), compoundend);
                      }
                      mystrcat(*result, presult, MAXLNLEN);
                      if (m || (*m != '\0')) {
                        op_sprintf(*result + op_strlen(*result), "%c%s%s%s", MSEP_FLD,
            ...
            if ((rv) && 
                ...
                && (
                   (!checkcompounddup || (rv != rv_first))
                      ...
                      if (m) op_free(m);
                      op_sprintf(*result + op_strlen(*result), "%c", MSEP_REC);
                      ok = 1;
            }
            ...
            if ((wordnum < maxwordnum) && (ok == 0)) {
                        compound_check_morph((word+i),op_strlen(word+i), wordnum+1, 
       ...
       int sfxopts, PfxEntry * ppfx, const FLAG needflag)
{
    ...
    char result3[MAXLNLEN]; // ARRAY OK 2009-02-09 jb (NOT CHECKED)
    ...
    unsigned short al, char * morph, char * targetmorph, int level)
{
    ...
    char mymorph[MAXLNLEN]; // ARRAY OK 2009-02-09 jb (NOT CHECKED)
...
//    int targetcount = get_sfxcount(targetmorph);
    ...
    if (op_strstr(morph, MORPH_INFL_SFX) || op_strstr(morph, MORPH_DERI_SFX)) {
        stemmorph = mymorph;
        op_strcpy(stemmorph, morph);
        mystrcat(stemmorph, " ", MAXLNLEN);
        stemmorphcatpos = stemmorph + op_strlen(stemmorph);
    } else {
        stemmorph = morph;
    ...
    for (int i = 0; i < al; i++) {
        ...
        while (sptr) {
            if (sptr->getFlag() == ap[i] && sptr->getMorph() && ((sptr->getContLen() == 0) || 
                ...
                if (stemmorphcatpos) op_strcpy(stemmorphcatpos, sptr->getMorph());
...
//                    (get_sfxcount(stemmorph) < targetcount) &&
                    ...
                    char * newword = sptr->add(ts, wl);
                    if (newword) {
                        char * newword2 = morphgen(newword, op_strlen(newword), sptr->getCont(),
    ...
    for (int i = 0; i < al; i++) {
       ...
       while (sptr) {
         if ((sptr->getFlag() == ap[i]) && (!sptr->getKeyLen() || ((badl > sptr->getKeyLen()) &&
                (op_strcmp(sptr->getAffix(), bad + badl - sptr->getKeyLen()) == 0))) &&
            ...
            if (newword) {
                if (nh < maxn) {
                    ...
                    // add special phonetic version
    		    if (phon && (nh < maxn)) {
    			char st[MAXWORDUTF8LEN];  // ARRAY OK 2009-02-25 jb
    			size_t phon_len = op_strlcpy(st, phon, sizeof(st));
    			if ( phon_len >= sizeof(st) || op_strlcat(st, sptr->getKey(), sizeof(st)) >= sizeof(st) )
					return nh - 1;
    			reverseword(st + phon_len);
          ...
          for (int k = 0; k < al; k++) {
             ...
             while (cptr) {
                if ((cptr->getFlag() == ap[k]) && cptr->allowCross() && (!cptr->getKeyLen() || ((badl > cptr->getKeyLen()) &&
                        (op_strncmp(cptr->getKey(), bad, cptr->getKeyLen()) == 0)))) {
                    int l1 = op_strlen(wlst[j].word);
    ...
    for (int m = 0; m < al; m ++) {
       ...
       while (ptr) {
         if ((ptr->getFlag() == ap[m]) && (!ptr->getKeyLen() || ((badl > ptr->getKeyLen()) &&
                (op_strncmp(ptr->getKey(), bad, ptr->getKeyLen()) == 0))) &&
                // check needaffix flag
...
int AffixMgr::parse_flag(char * line, unsigned short * out, FileMgr * af) {
   ...
   if (*out != FLAG_NULL && !(*out >= DEFAULTFLAGS)) {
      HUNSPELL_WARNING(hunspell_stderr, "error: line %d: multiple definitions of an affix file parameter\n", af->getlinenum());
      return 1;
   }
...
int  AffixMgr::parse_cpdsyllable(char * line, FileMgr * af)
{
   ...
   while (piece) {
      if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { cpdmaxsyllable = op_atoi(piece); np++; break; }
             case 2: {
                if (!utf8) {
                    ...
                    if (n > 0) {
                        ...
                        cpdvowels_utf16 = (w_char *) op_malloc(n * sizeof(w_char));
                        if (!cpdvowels_utf16) return 1;
                        op_memcpy(cpdvowels_utf16, w, n * sizeof(w_char));
                    }
   ...
   if (np < 2) {
      HUNSPELL_WARNING(hunspell_stderr, "error: line %d: missing compoundsyllable information\n", af->getlinenum());
      return 1;
   }
   ...
   while (piece) {
       if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { 
                       numrep = op_atoi(piece);
                       if (numrep < 1) {
                          HUNSPELL_WARNING(hunspell_stderr, "error: line %d: incorrect entry number\n", af->getlinenum());
                          return 1;
                       }
                       reptable = (replentry *) op_malloc(numrep * sizeof(struct replentry));
           ...
           if (*piece != '\0') {
               switch(i) {
                  ...
                  case 1: {
                            ...
                            int lr = op_strlen(reptable[j].pattern) - 1;
   ...
   for (int j=0; j < numrl; j++) {
        ...
        while (piece) {
           if (*piece != '\0') {
               switch(i) {
                  case 0: {
                             if (op_strncmp(piece, keyword, sizeof(keyword)) != 0) {
                                 HUNSPELL_WARNING(hunspell_stderr, "error: line %d: table is corrupt\n", af->getlinenum());
                                 OP_DELETE( *rl);
                                 *rl = NULL;
                                 return 1;
   ...
   while (piece) {
       if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { 
                       phone = (phonetable *) op_malloc(sizeof(struct phonetable));
                       if (!phone) return 1;
                       phone->num = op_atoi(piece);
                       phone->rules = NULL;
                       ...
                       phone->rules = (char * *) op_malloc(2 * (phone->num + 1) * sizeof(char *));
                       if (!phone->rules) {
                          op_free(phone);
                          phone = NULL;
                          return 1;
   ...
   while (piece) {
       if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { 
                       numcheckcpd = op_atoi(piece);
                       if (numcheckcpd < 1) {
                          HUNSPELL_WARNING(hunspell_stderr, "error: line %d: bad entry number\n", af->getlinenum());
                          return 1;
                       }
                       checkcpdtable = (patentry *) op_malloc(numcheckcpd * sizeof(struct patentry));
   ...
   for (int j=0; j < numcheckcpd; j++) {
        ...
        while (piece) {
           if (*piece != '\0') {
               switch(i) {
                  case 0: {
                             if (op_strncmp(piece,"CHECKCOMPOUNDPATTERN",20) != 0) {
                                 HUNSPELL_WARNING(hunspell_stderr, "error: line %d: table is corrupt\n", af->getlinenum());
                                 numcheckcpd = 0;
                                 return 1;
                             }
                  ...
                  case 1: { 
                    ...
                    char * p = op_strchr(checkcpdtable[j].pattern, '/');
                    if (p) {
                      *p = '\0';
                  ...
                  case 2: { 
                    ...
                    char * p = op_strchr(checkcpdtable[j].pattern2, '/');
                    if (p) {
                      *p = '\0';
   ...
   while (piece) {
       if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { 
                       numdefcpd = op_atoi(piece);
                       if (numdefcpd < 1) {
                          HUNSPELL_WARNING(hunspell_stderr, "error: line %d: bad entry number\n", af->getlinenum());
                          return 1;
                       }
                       defcpdtable = (flagentry *) op_malloc(numdefcpd * sizeof(flagentry));
   ...
   for (int j=0; j < numdefcpd; j++) {
        ...
        while (piece) {
           if (*piece != '\0') {
               switch(i) {
                  case 0: {
                             if (op_strncmp(piece, "COMPOUNDRULE", 12) != 0) {
                                 HUNSPELL_WARNING(hunspell_stderr, "error: line %d: table is corrupt\n", af->getlinenum());
                                 numdefcpd = 0;
                                 return 1;
                             }
                  ...
                  case 1: { // handle parenthesized flags
                            if (op_strchr(piece, '(')) {
                                defcpdtable[j].def = (FLAG *) op_malloc(op_strlen(piece) * sizeof(FLAG));
                                defcpdtable[j].len = 0;
                                int end = 0;
                                FLAG * conv;
                                while (!end) {
   ...
   while (piece) {
       if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { 
                       nummap = op_atoi(piece);
                       if (nummap < 1) {
                          HUNSPELL_WARNING(hunspell_stderr, "error: line %d: bad entry number\n", af->getlinenum());
                          return 1;
                       }
                       maptable = (mapentry *) op_malloc(nummap * sizeof(struct mapentry));
        ...
        while (piece) {
           if (*piece != '\0') {
               switch(i) {
                  ...
                  case 1: {
			    int setn = 0;
                            maptable[j].len = op_strlen(piece);
                            maptable[j].set = (char **) op_malloc(maptable[j].len * sizeof(char*));
                            if (!maptable[j].set) return 1;
			    for (int k = 0; k < maptable[j].len; k++) {
				int chl = 1;
				int chb = k;
			        if (piece[k] == '(') {
				    char * parpos = op_strchr(piece + k, ')');
				    if (parpos != NULL) {
					chb = k + 1;
				...
				maptable[j].set[setn] = (char *) op_malloc(chl + 1);
				if (!maptable[j].set[setn]) return 1;
				hunspell_strncpy(maptable[j].set[setn], piece + chb, chl);
   ...
   while (piece) {
       if (*piece != '\0') {
          switch(i) {
             case 0: { np++; break; }
             case 1: { 
                       numbreak = op_atoi(piece);
                       if (numbreak < 0) {
                          HUNSPELL_WARNING(hunspell_stderr, "error: line %d: bad entry number\n", af->getlinenum());
                          return 1;
                       }
                       if (numbreak == 0) return 0;
                       breaktable = (char **) op_malloc(numbreak * sizeof(char *));
                       if (!breaktable) return 1;
                       np++;
                       break;
                     }
...
void AffixMgr::reverse_condition(char * piece) {
    int neg = 0;
    for (char * k = piece + op_strlen(piece) - 1; k >= piece; k--) {
        switch(*k) {
          case '[': {
                if (neg) *(k+1) = '['; else *k = ']';
                    break;
...
/** Opera addition: Using custom vector class to avoid dependencies on STL. Same API, much smaller. Non-exception based oom checks. */
class affentry_vector
{
public:
  affentry_vector() : storage(NULL), storage_len(0) {}
  ~affentry_vector() { OP_DELETEA(storage); }
  typedef affentry* iterator;
  iterator begin() { return storage; }
  iterator end() { return storage + storage_len; }
  affentry& operator[](int i) { return storage[i]; }
  bool resize(int new_size)
  {
    affentry* new_array = OP_NEWA(affentry, new_size);
    if (!new_array)
      return false;
    int common_size = MIN(new_size, storage_len);
    for (int i = 0; i < common_size; i++)
      new_array[i] = storage[i];
    OP_DELETEA(storage);
    storage = new_array;
    storage_len = new_size;
    return true;
  }
private:
  affentry* storage;
...
int  AffixMgr::parse_affix(char * line, const char at, FileMgr * af, char * dupflags)
{
   ...
   char ff=0;
   // Opera change: original hunspell 1.3.1 uses std:vector which isn't portable enough for us.
   affentry_vector affentries;
          ...
          switch(i) {
             ...
             case 1: { 
                    ...
                    if (((at == 'S') && (dupflags[aflag] & dupSFX)) ||
                        ...
                        HUNSPELL_WARNING(hunspell_stderr, "error: line %d: multiple definitions of an affix flag\n",
                            af->getlinenum());
   ...
   affentry_vector::iterator start = affentries.begin();
   affentry_vector::iterator end = affentries.end();
   for (affentry_vector::iterator entry = start; entry != end; ++entry) {
      if (!(nl = af->getline())) return 1;
      mychomp(nl);
      tp = nl;
      i = 0;
             ...
             switch(i) {
                ...
                case 1: { 
                          ...
                          if (pHMgr->decode_flag(piece) != aflag) {
                              char * err = pHMgr->encode_flag(aflag);
                              if (err) {
                                HUNSPELL_WARNING(hunspell_stderr, "error: line %d: affix %s is corrupt\n",
                                    af->getlinenum(), err);
                                op_free(err);
                              }
                              return 1;
                ...
                case 2: { 
                          ...
                          entry->stripl = (unsigned char) op_strlen(entry->strip);
                          if (op_strcmp(entry->strip,"0") == 0) {
                              op_free(entry->strip);
                ...
                case 3: { 
                          ...
                          if (dash) {
                            *dash = '\0';
                            ...
                            if (pHMgr->is_aliasf()) {
                                int index = op_atoi(dash + 1);
                                entry->contclasslen = (unsigned short) pHMgr->get_aliasf(index, &(entry->contclass), af);
                                if (!entry->contclasslen) HUNSPELL_WARNING(hunspell_stderr, "error: bad affix flag alias: \"%s\"\n", dash+1);
                            } else {
                          ...
                          entry->appndl = (unsigned char) op_strlen(entry->appnd);
                          if (op_strcmp(entry->appnd,"0") == 0) {
                              op_free(entry->appnd);
                ...
                case 4: { 
                          ...
                          if (entry->stripl && (op_strcmp(piece, ".") != 0) &&
      ...
      if (basefieldnum) {
        int fieldnum = !(entry->morphcode) ? 5 : ((*(entry->morphcode)=='#') ? 5 : 6);
          if (fieldnum != basefieldnum) 
            HUNSPELL_WARNING(hunspell_stderr, "warning: line %d: bad field number\n", af->getlinenum());
      } else {
   ...
   for (affentry_vector::iterator entry = start; entry != end; ++entry) {
      if (at == 'P') {
          PfxEntry * pfxptr = OP_NEW(PfxEntry, (this,&(*entry)));
          if (!pfxptr)
              return 1;
          build_pfxtree(pfxptr);
      } else {
          SfxEntry * sfxptr = OP_NEW(SfxEntry, (this,&(*entry)));
          if (!sfxptr)
              return 1;
...
int AffixMgr::redundant_condition(char ft, char * strip, int stripl, const char * cond, int linenum) {
  int condl = op_strlen(cond);
  int i;
  int j;
  int neg;
  int in;
  if (ft == 'P') { // prefix
    if (op_strncmp(strip, cond, condl) == 0) return 1;
    if (utf8) {
    } else {
      for (i = 0, j = 0; (i < stripl) && (j < condl); i++, j++) {
        if (cond[j] != '[') {
          if (cond[j] != strip[i]) {
            HUNSPELL_WARNING(hunspell_stderr, "warning: line %d: incompatible stripping characters and condition\n", linenum);
            return 0;
          }
        } else {
          neg = (cond[j+1] == '^') ? 1 : 0;
          ...
          if (j == (condl - 1) && (cond[j] != ']')) {
            HUNSPELL_WARNING(hunspell_stderr, "error: line %d: missing ] in condition:\n%s\n", linenum, cond);
            return 0;
          }
          if ((!neg && !in) || (neg && in)) {
            HUNSPELL_WARNING(hunspell_stderr, "warning: line %d: incompatible stripping characters and condition\n", linenum);
    ...
    if ((stripl >= condl) && op_strcmp(strip + stripl - condl, cond) == 0) return 1;
    if (utf8) {
    } else {
      for (i = stripl - 1, j = condl - 1; (i >= 0) && (j >= 0); i--, j--) {
        if (cond[j] != ']') {
          ...
          if ((j == 0) && (cond[j] != '[')) {
            HUNSPELL_WARNING(hunspell_stderr, "error: error: %d: missing ] in condition:\n%s\n", linenum, cond);
            return 0;
          }
          neg = (cond[j+1] == '^') ? 1 : 0;
          if ((!neg && !in) || (neg && in)) {
