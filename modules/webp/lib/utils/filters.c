...
#define SMAX 16
#define SDIFF(a, b) ((vp8_abs((a) - (b)) >> 4) & 0xf)   // Scoring diff, in [0..SMAX)

WEBP_FILTER_TYPE EstimateBestFilter(const uint8_t* data,
                                    int width, int height, int stride) {
  int i, j;
  int bins[WEBP_FILTER_LAST][SMAX]; // ARRAY OK 2012-08-22 andersr
  ...
  {
    /* WEBP_FILTER_TYPE */ int filter, best_filter = WEBP_FILTER_NONE;
    ...
    return (WEBP_FILTER_TYPE)best_filter;
  }
}
